...
tripRoute.get("/schedules", async (req, res) => {
    try {
        // Get all routes with their schedules and buses
        const schedules = await (0, db_1.default)("SCHEDULE")
            .join("ROUTE", "SCHEDULE.RouteID", "=", "ROUTE.RouteID")
            .select("SCHEDULE.ScheduleID", "SCHEDULE.RouteID", "ROUTE.RouteName", "SCHEDULE.ArrivalTime", "SCHEDULE.DepartureTime", "SCHEDULE.DaysOfWeek")
...
tripRoute.get("/routes/:routeId", async (req, res) => {
    try {
        const { routeId } = req.params;
        // Get route info
        const route = await (0, db_1.default)("ROUTE")
        ...
        const stops = await (0, db_1.default)("ROUTE_STOPS")
            .join("STOPS", "ROUTE_STOPS.StopID", "=", "STOPS.StopID")
            .where({ RouteID: routeId })
            .select("STOPS.StopID", "STOPS.StopName", "STOPS.Latitude", "STOPS.Longitude", "ROUTE_STOPS.StopOrder")
...
tripRoute.get("/route/:routeId/trips", async (req, res) => {
    try {
        const { routeId } = req.params;
        const trips = await (0, db_1.default)("TRIP_INSTANCE")
            ...
            .select("TRIP_INSTANCE.TripInstanceID", "TRIP_INSTANCE.ActualDate", "TRIP_INSTANCE.ActualStartTime", "TRIP_INSTANCE.ActualEndTime", "TRIP_INSTANCE.PassengerCount", "BUS.PlateNumber", "BUS.Capacity", "BUS.BusType")
...
// 4. Get trip details
tripRoute.get("/trips/:tripInstanceId", async (req, res) => {
    try {
        const { tripInstanceId } = req.params;
        const trip = await (0, db_1.default)("TRIP_INSTANCE")
            .where({ TripInstanceID: tripInstanceId })
            .join("SCHEDULE", "TRIP_INSTANCE.ScheduleID", "=", "SCHEDULE.ScheduleID")
            .join("BUS", "TRIP_INSTANCE.BusID", "=", "BUS.BusID")
            .join("DRIVER", "TRIP_INSTANCE.DriverID", "=", "DRIVER.DriverID")
            .select("TRIP_INSTANCE.*", "SCHEDULE.RouteID", "SCHEDULE.DepartureTime as scheduledDepartureTime", "SCHEDULE.ArrivalTime as scheduledArrivalTime", "BUS.PlateNumber", "BUS.Capacity", "BUS.BusType", "DRIVER.Name as DriverName")
            .first();
        if (!trip) {
...
tripRoute.post("/trips/:tripInstanceId/buy-ticket", authenticator_1.default, async (req, res) => {
    try {
        const { tripInstanceId } = req.params;
        ...
        const updatedCard = await (0, db_1.default)("SMART_CARD")
            .where({ CardID: card.CardID })
            .first();
        return res.json({
...
tripRoute.get("/my-bookings", authenticator_1.default, async (req, res) => {
    try {
        const passengerId = req.user.PassengerID;
        // Get all trips the passenger has booked
        const bookings = await (0, db_1.default)("TRANSACTION")
            .join("SMART_CARD", "TRANSACTION.CardID", "=", "SMART_CARD.CardID")
            ...
            .select("TRANSACTION.TransactionID", "TRANSACTION.Amount", "TRANSACTION.TimeStamp as bookedAt", "TRIP_INSTANCE.TripInstanceID", "TRIP_INSTANCE.ActualDate", "TRIP_INSTANCE.ActualStartTime", "TRIP_INSTANCE.ActualEndTime", "ROUTE.RouteName", "BUS.PlateNumber", "BUS.BusType")
...
tripRoute.post("/trips/:tripInstanceId/cancel", authenticator_1.default, async (req, res) => {
    try {
        const { tripInstanceId } = req.params;
        const passengerId = req.user.PassengerID;
